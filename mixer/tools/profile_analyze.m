...
%				checkpoints{k} = sprintf("%-50s   (%s)", name, checkpoints{k});
	...
	% attention: don't use [cp.priority] or cell2mat({cp.priority}) because NaNs become 0 there
	I = find([cellfun(@isnan, {cp.priority})]);
	if (!isempty(I))
		printf("warning: following checkpoints are not found in profile info and will be removed:\n");
		cellfun(@(x) printf("\t%s\n", x), checkpoints(I));
		checkpoints(I) = [];
		cp(I) = [];
		profile(I,:) = [];
