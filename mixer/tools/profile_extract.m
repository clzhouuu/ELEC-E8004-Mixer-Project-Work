...
function [fLineHeader, testbed, setup, rounds, traceMap] = scanTraceFile(fileName)
	...
	if (1)
		...
		if (!isempty(s1))
			s2 = regexp(s, [fLineHeader(p{1}) "INFOS \\(\\d+\\)"], "matches", "once");
			if (isempty(s2))
				fmt = 3;
			endif
		endif
	...
	switch (fmt)
		...
		case 3
			setup.fmtINFOS = "rank=\\d+.*?preparing round (?<round>\\d+)";
	endswitch
	...
	for i = 1 : length(p)
		traceMap(nodesMap(str2double(s1(i).id)), str2double(s1(i).round) - (fmt == 3)) = p(i) - 1;
	endfor
