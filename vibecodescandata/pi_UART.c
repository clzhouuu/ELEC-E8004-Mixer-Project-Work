...
static PiScanFrag_t   g_latest_frag;
static uint8_t        g_fresh_frag = 0;
...
// Frame processing
// -----------------------------------------------------------------------
static void process_pose_frame(uint32_t now_ms) {
    PiPayload_t pose;
...
static void process_scan_frag_frame(uint32_t now_ms) {
    (void)now_ms;

    if (g_length != sizeof(PiScanFrag_t)) {
        return;
    }

    memcpy(&g_latest_frag, g_payload, sizeof(PiScanFrag_t));
    g_fresh_frag = 1;
}
    ...
    case PI_WAIT_CHECKSUM:
        if (byte == g_checksum) {
            if (g_msg_id == PI_UART_MSG_POSE) {
                process_pose_frame(now_ms);
            } else if (g_msg_id == PI_UART_MSG_SCAN_FRAG) {
                process_scan_frag_frame(now_ms);
            }
        }
        parser_reset();
        break;
...
void pi_uart_init(uint8_t robot_id) {
    ...
    memset(&g_latest_frag, 0, sizeof(g_latest_frag));
    g_fresh_pose = 0;
    g_fresh_frag = 0;
    parser_reset();
    ...
    // 115200 baud @ 12 MHz SMCLK, oversampling mode
    // N = 12000000 / 115200 = 104.17
    // BRDIV = floor(104.17 / 16) = 6
    // BRF   = round((104.17/16 - 6) * 16) = 8
    // BRS   = 0x20  (from TI UCBRSx table for remainder 0.0104)
...
void pi_uart_send_pose(const RobotPoseMsg_t *pose) {
    ...
    uint8_t msg_id  = PI_UART_MSG_PEER_POSE;
